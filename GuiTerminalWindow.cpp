...
{
    mainWindowMode = mainWindow ? true : false;
...
int GuiTerminalWindow::initTerminal()
{
    ...
    if (error) {
        ...
        QString msg = "Unable to open connection to \n %1 \n %2";
        msg.arg(cfg_dest(&cfg)).arg(error);
        qt_message_box(this, APPNAME " Error", msg.toStdString().c_str());
        backend = NULL;
        term = NULL;
        ldisc = NULL;
        goto cu0;
    ...
    if (mainWindowMode && !(mainWindow->windowState() & Qt::WindowMaximized) &&
                                  ...
                                  const QString &str, unsigned long attr)
{
    ...
    if ((attr & TATTR_PASCURS) && (ctype == 0 || term->big_cursor)) {
        ...
        if (attr & TATTR_ACTCURS) {
            qutty_assert(0); // NOT_YET_IMPLEMENTED
        } else {
            painter.setPen(colours[261]);
            for (i = 0; i < length; i++) {
                if (i % 2 == 0) {
...
void 	GuiTerminalWindow::mouseDoubleClickEvent ( QMouseEvent * e )
{
    ...
    qutty_assert(button!=MBT_NOTHING);
...
{
    // TODO version check
    qutty_assert(_tmuxMode == TMUX_MODE_NONE);
...
{
    qutty_assert(_tmuxMode == TMUX_MODE_GATEWAY_DETACH_INIT);
