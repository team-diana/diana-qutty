...
#define add(r,x,y) ( r.lo = y.lo + x.lo, \
		     r.hi = y.hi + x.hi + ((uint32_t)r.lo < (uint32_t)y.lo) )
#define rorB(r,x,y) ( r.lo = ((uint32_t)x.hi >> ((y)-32)) | ((uint32_t)x.lo << (64-(y))), \
		      r.hi = ((uint32_t)x.lo >> ((y)-32)) | ((uint32_t)x.hi << (64-(y))) )
#define rorL(r,x,y) ( r.lo = ((uint32_t)x.lo >> (y)) | ((uint32_t)x.hi << (32-(y))), \
		      r.hi = ((uint32_t)x.hi >> (y)) | ((uint32_t)x.lo << (32-(y))) )
#define shrB(r,x,y) ( r.lo = (uint32_t)x.hi >> ((y)-32), r.hi = 0 )
#define shrL(r,x,y) ( r.lo = ((uint32_t)x.lo >> (y)) | ((uint32_t)x.hi << (32-(y))), \
		      r.hi = (uint32_t)x.hi >> (y) )
