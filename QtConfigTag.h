...
#define QUTTY_SERIALIZE_STRUCT_CONFIG_ELEMENT_LIST \
    ...
    int(tcp_nodelay) \
    int(tcp_keepalives) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, loghost, 512)  /* logical host being contacted, for host key check */ \
    /* Proxy options */ \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, proxy_exclude_list, 512) \
    int(proxy_dns) \
    int(even_proxy_localhost) \
    int(proxy_type) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, proxy_host, 512) \
    int(proxy_port) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, proxy_username, 128) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, proxy_password, 128) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, proxy_telnet_command, 512) \
    /* SSH options */ \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, remote_cmd, 512) \
    int(nopty) \
    int(compression) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(int, ssh_kexlist, KEX_MAX) \
    int(ssh_rekey_time)		       /* in minutes */ \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, ssh_rekey_data, 16) \
    int(tryagent) \
    int(agentfwd) \
    ...
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, username, 100) \
    int(username_from_env) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, localusername, 100) \
    int(rfc_environ) \
    int(passive_telnet) \
    /* Serial port options */ \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, serline, 256) \
    int(serspeed) \
    int(serdatabits) int(serstopbits) \
    int(serparity) \
    int(serflow) \
    /* Keyboard options */ \
    int(bksp_is_delete) \
    ...
    int(compose_key) \
    int(ctrlaltkeys) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, wintitle, 256)		       /* initial window title */ \
    /* Terminal options */ \
    int(savelines) \
    int(dec_om) \
    int(wrap_mode) \
    ...
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, printer, 128) \
    int(arabicshaping) \
    int(bidi) \
    /* Colour options */ \
    int(ansi_colour) \
    ...
    int(rtf_paste) \
    int(mouse_override) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(short, wordness, 256) \
    /* translations */ \
    int(vtmode) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, line_codepage, 128) \
    int(cjk_ambig_wide) \
    int(utf8_override) \
    int(xlat_capslockcyr) \
    /* X11 forwarding */ \
    int(x11_forward) \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, x11_display, 128) \
    int(x11_auth) \
    Filename(xauthfile) \
    /* port forwarding */ \
    int(lport_acceptall) /* accept conns from hosts other than localhost */ \
    int(rport_acceptall) /* same for remote forwarded ports (SSH-2 only) */ \
    QUTTY_SERIALIZE_ELEMENT_ARRAY(char, portfwd, 1024) \
    /* SSH bug compatibility modes */ \
    int(sshbug_ignore1) int(sshbug_plainpw1) int(sshbug_rsa1) \
    int(sshbug_hmac2) int(sshbug_derivekey2) int(sshbug_rsapad2) \
    int(sshbug_pksessid2) int(sshbug_rekey2) int(sshbug_maxpkt2) \
    int(sshbug_ignore2) \
    int(ssh_simple) \
